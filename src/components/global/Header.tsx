...
const Header = ({ toggleMenu, className, title, audio, cart }: HeaderProps) => {
  ...
  return (
    ...
    >
      ...
      <h1 className="absolute w-[100dvw] left-0 z-30 text-white font-bold bg-black p-8P pl-0 -mt-8M sm:mt-0 sm:p-16P sm:pl-0 text-center text-lg sm:text-2xl lg:text-3xl">
        {title}
      </h1>
      <div className="flex justify-center items-center">
        {audio === 'true' && (
          <button
            onClick={toggleAudio}
            className="relative z-50 text-white font-bold text-3xl"
          >
            {isPlaying ? (
        ...
        {cart === 'true' && (
          <Link
            href="/comanda"
            className="relative z-50 text-white font-bold text-3xl group"
          >
